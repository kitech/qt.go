...
extern void* C_ZN7QBitmapC2ERKS_(void* arg0); // 1
...
func NewQBitmap(args ...interface{}) *QBitmap {
  ...
  case 1:
    // invoke: _ZN7QBitmapC1ERKS_
    // invoke: void QBitmap(const class QBitmap &)
    var arg0 = args[0].(*QBitmap).Qclsinst
    if false {fmt.Println(arg0)}
    var qthis = unsafe.Pointer(C.malloc(5))
    if false {reflect.TypeOf(qthis)}
    qthis = C.C_ZN7QBitmapC2ERKS_(arg0)
    return &QBitmap{Qclsinst:qthis}
  case 2:
